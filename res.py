...
@hydra.main(
    config_path=f"{path}\\config", config_name="visualization", version_base="1.3"
)
def main(cfg: DictConfig):

    K0_coll = np.arange(cfg.min_sparsity, cfg.max_sparsity, cfg.sparsity_freq)
    dict_types = ["separated"]  # ["separated", "edge", "joint"]
    ...
    if res_type == "real":
        real_res_path = f"{path}\\results\\real\\real_J{cfg.J}P{cfg.P}.pkl"
        curves_params = {
            "j": cfg.J,
            "p": cfg.P,
        ...
        plot_error_curves_real(real_res, K0_coll, **curves_params)
