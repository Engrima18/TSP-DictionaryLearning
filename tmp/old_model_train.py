...
def sparse_transform(D, K0, Y_test, Y_train=None):
    ...
    X_test = np.apply_along_axis(
        lambda x: get_omp_coeff(K0, Domp=Domp, col=x), axis=0, arr=Y_test
    )
    # Normalization
    X_test = W @ X_test
    ...
    if Y_train != None:
        X_train = np.apply_along_axis(
            lambda x: get_omp_coeff(K0, Domp=Domp, col=x), axis=0, arr=Y_train
        )
        X_train = W @ X_train
    ...
    if (only == "only_D") or (only == "all"):
        ...
        while discard == 1:
            ...
            if dictionary_type == "joint":
                ...
                h, discard = _multiplier_search(
                    lambda_max_j, lambda_min_j, P=P, c=c, epsilon=epsilon
                )
                ...
                Luj, lambda_max_u_j, lambda_min_u_j = compute_Lj_and_lambdaj(
                    Lu, J, separated=True
                )
                Ldj, lambda_max_d_j, lambda_min_d_j = compute_Lj_and_lambdaj(
                    Ld, J, separated=True
                )
                h, discard = _multiplier_search(
                    lambda_max_d_j,
    ...
    if only == "only_X" or only == "all":
        ...
        if h == None:
            ...
            X = np.apply_along_axis(
                lambda x: get_omp_coeff(K0, Domp.real, x), axis=0, arr=Y_train
            )
            X = np.tile(X, (P, 1))
...
def topological_dictionary_learn(
    ...
    if dictionary_type != "fourier":
        ...
        while pat_iter < patience and iter_ <= max_iter:
            ...
            if dictionary_type != "separated":
                ...
                for i in range(0, P):
                    tmp = cp.Constant(np.zeros((M, M)))
                    for j in range(0, J):
                        tmp += (cp.Constant(Luj[j, :, :]) * hS[i, j]) + (
                            cp.Constant(Ldj[j, :, :]) * hI[i, j]
                        )
                    tmp += I * hH[i]
                    D_coll[i] = tmp
                    Dsum += tmp
            ...
            constraints = (
                [D_coll[i] >> 0 for i in range(P)]
                + [(cp.multiply(c, I) - D_coll[i]) >> 0 for i in range(P)]
                + [
                    (Dsum - cp.multiply((c - epsilon), I)) >> 0,
                    (cp.multiply((c + epsilon), I) - Dsum) >> 0,
                ]
            )
            ...
            # Error Updating
            error_train = (1 / m_train) * np.sum(
                la.norm(Y_train - (D @ X_train), axis=0) ** 2
                / la.norm(Y_train, axis=0) ** 2
            )
            error_test = (1 / m_test) * np.sum(
                la.norm(Y_test - (D @ X_test), axis=0) ** 2
                / la.norm(Y_test, axis=0) ** 2
            )

            # Error Storing
            if (
                (error_train < min_error_train)
                and (abs(error_train) > np.finfo(float).eps)
                and (abs(error_train - min_error_train) > tol)
            ):
                X_opt_train = X_train
                min_error_train = error_train

            if (
                (error_test < min_error_test)
                and (abs(error_test) > np.finfo(float).eps)
                and (abs(error_test - min_error_test) > tol)
            ):
                h_opt = (
                    h.value
                    if dictionary_type in ["joint", "edge_laplacian"]
                    else np.hstack([hI.value, hS.value, hH.value])
                )
                D_opt = D
                X_opt_test = X_test
        ...
        Domp = D_opt @ W
        X_opt_train = np.apply_along_axis(
            lambda x: get_omp_coeff(K0, Domp=Domp.real, col=x), axis=0, arr=Y_train
        )
        X_opt_test = np.apply_along_axis(
            lambda x: get_omp_coeff(K0, Domp=Domp.real, col=x), axis=0, arr=Y_test
        )
        X_opt_train = W @ X_opt_train
        X_opt_test = W @ X_opt_test
        # Error Updating
        min_error_train = (1 / m_train) * np.sum(
            la.norm(Y_train - (D_opt @ X_opt_train), axis=0) ** 2
            / la.norm(Y_train, axis=0) ** 2
        )
        min_error_test = (1 / m_test) * np.sum(
            la.norm(Y_test - (D_opt @ X_opt_test), axis=0) ** 2
            / la.norm(Y_test, axis=0) ** 2
        )
        h_opt = 0
