...
def dict_and_topology_learning(
    ...
    dict_errors = {
        "pess": (min_error_pess_train, min_error_pess_test, approx_pess),
    }
    ...
    if algo_params["both"]:
        min_error_opt_train = np.zeros((n_sim, len(K0_coll)))
        min_error_opt_test = np.zeros((n_sim, len(K0_coll)))
        approx_opt = np.zeros((n_sim, len(K0_coll)))
        dict_errors["opt"] = (
            min_error_opt_train,
            min_error_opt_test,
            approx_opt,
        ...
        dict_types["opt"] = (
            "Separated Hodge Laplacian with optimistic topology learning",
            "separated",
        )
