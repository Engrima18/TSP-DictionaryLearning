...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run the TSP simulation with customizable parameters."
    )
    parser.add_argument(
        "--dictionary_type",
        type=str,
        default="separated",
        help="Can be of type: separated, edge_laplacian or joint",
    )
    parser.add_argument(
        "--n_sim", type=int, default=10, help="Number of simulations to run"
    )
    parser.add_argument(
        "--n", type=int, default=40, help="Number of nodes in the graph"
    )
    parser.add_argument(
        "--p",
        type=float,
        default=0.162,
        help="Probability for edge creation in the graph",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Sub-sampling size to decrease complexity",
    )
    parser.add_argument(
        "--m_train", type=int, default=150, help="Number of Train Signals"
    )
    parser.add_argument("--m_test", type=int, default=80, help="Number of Test Signals")
    parser.add_argument(
        "--s", type=int, default=3, help="Number of Kernels (Sub-dictionaries)"
    )
    parser.add_argument("--k", type=int, default=2, help="Polynomial order")
    parser.add_argument(
        "--sparsity", type=float, default=0.1, help="Sparsity percentage"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3000,
        help="Number of searches for best sparse approximation",
    )
    parser.add_argument(
        "--verbose",
        type=bool,
    ...
    parser.add_argument(
        "--tol", type=float, default=1e-7, help="Tolerance for patience"
    )
    parser.add_argument(
        "--lambda_",
        type=float,
        default=1e-7,
        help="l2 multiplier for the optimization problem",
    )
    parser.add_argument(
        "--max_iter",
        type=int,
        default=100,
        help="Max number of iterations for the optimization algorithm",
    )
    parser.add_argument(
        "--patience",
        type=int,
        default=1e-7,
        help="Patience of the optimization algorithm",
    )
...
def run_simulation(args):
    ...
    initialize_simulation_variables(
        args.n, args.s, args.n_sim, args.m_train, args.m_test, args.n_search
    )
    ...
    for sim in range(args.n_sim):
        ...
        for _ in tqdm(range(args.n_search)):
            try:
                ...
                max_possible_sparsity, acc = verify_dic(
                    D_try, Y_train_try, X_train_try, args.K0_max, 0.7
                )
        ...
        for k0_index, k0 in tqdm(enumerate(K0_coll)):
            ...
            for d in dict_types.items():
                try:
                    ...
                    if args.verbose:
                        print(
                            f"Simulation: {sim+1}/{args.n_sim} Sparsity: {k0} Testing {d[1][0]}... Done! Test Error: {dict_errors[d[0]][1][sim,k0_index]}"
                        )
                except:
                    print(
                        f"Simulation: {sim+1}/{args.n_sim} Sparsity: {k0} Testing {d[1][0]}... Diverged!"
                    )
                    try:
                        (
                            dict_errors[d[0]][0][sim, k0_index],
